 ...
 *  - Each phase-folded view has an ImgBB share button.
 */
...
export default function ExoMinerPanel({
  ...
  // Build a descriptive ImgBB filename/title for shared views.
  const ticId = result.star.tic_id;
  const sector = result.star.sector;
  const titleFor = (label: string) =>
    ticId
      ? `TIC ${ticId}${sector ? ` S${sector}` : ""} — ${label}`
      : label;
